...
def calculate_gene_prevalences(gene_depth_matrix, marker_coverages, min_copynum=0.3, max_copynum=3.0):

    gene_copynum_matrix = gene_depth_matrix * 1.0 / numpy.clip(marker_coverages,1,1e09)

    return ((gene_copynum_matrix >= min_copynum) & (gene_copynum_matrix <= max_copynum)).sum(axis=1)


def calculate_fractional_gene_prevalences(gene_depth_matrix, marker_coverages, min_copynum=0.3, max_copynum=3.0):

    return calculate_gene_prevalences(gene_depth_matrix, marker_coverages, min_copynum, max_copynum)*1.0/len(marker_coverages)
