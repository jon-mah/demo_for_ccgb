...
class DemographicInference():
    ...
    def fitDemographicModelParser(self):
        ...
        parser.add_argument(
            'projection_size', type=int,
            help=('Size used for projection of downsampled SFS.'))
        return parser
    ...
    def main(self):
        ...
        downsampled_sfs = syn_input_sfs.replace('.sfs', '_downsampled_' + str(args['projection_size']) + '.sfs')
        to_remove = [downsampled_sfs]
        for f in to_remove:
            if os.path.isfile(f):
                os.remove(f)
        ...
        syn_data = Spectrum([allele_counts*passed_sites_matrix, axis=1])

        proj_data = syn_data.project([projection_size])
        
        proj_data = numpy.asarray(proj_data)
        proj_data[0] = 0
        masked = (proj_data <= 0) + 0
        masked[0] = 1
        species_name = syn_input_sfs[8:-8]
        # print(proj_data)
        # print(str(projection_size) + ' folded "' + species_name + '"')
        # second_string = ' '.join(str(proj_data))
        # print(second_string)
        second_string = str(proj_data).replace('\t', ' ')
        second_string = str(proj_data)[1:-1]
        second_string.replace('\t', ' ')
        third_string = str(masked)[1:-1]
        print(second_string)
        print(third_string)
        with open(downsampled_sfs, 'w+') as f:
            f.write(str(projection_size) + ' folded "' + species_name + '"')
            f.write('\n' + second_string)
            f.write('\n' + third_string)

        print(downsampled_sfs)
        logger.info('Finished downsampling projection.')
